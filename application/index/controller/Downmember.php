    ...
    public function login () {
        ...
        if($result===false){  
           return $this->error('验证码错误！',Url('my/login'));exit;  
            ...
            if (empty($is_member)) {
                return $this->error('账号密码有误',Url('downmember/login'));
            }else{
                Session::set('userid',$is_member['userid']);
                Session::set('username',$is_member['username']);
                return $this->error('登陆成功',Url('downmember/profile'));
            }
        }
        return $this->fetch();
    }
...
//退出登录
    public function logout(){
        session(null); 
        $this->success('退出系统成功！',url('downmember/login'));
    }
...
//修改用户资料
    public function profile(){
      if (!empty($_POST)){
      $data = input("post.");
      $save=$member->save($data,['userid'=>$data['userid']]);
      if (!empty($save)) {

        $this->success("修改成功！",'downmember/profile');
      }else{
        $this->error("修改失败！！！");
      }
      
      }
      $downmembers=DownmemberModel::find(input('userid'));
